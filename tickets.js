...
function patchTickets(coreSource) {
  ...
  const imports = `import { db, fs } from ${JSON.stringify(FIREBASE_URL)};\nimport { listEmployees } from ${JSON.stringify(DIRECTORY_URL)};\nimport { runtimeAssignmentCandidates, runtimeCanOpenTickets, runtimeCanSetTicketAssignment, runtimeCanViewTicket, runtimeEscalationTarget, runtimeTicketProjectId, runtimeTicketScope } from ${JSON.stringify(POLICY_URL)};`;
  ...
  source = replaceRequired(
    ...
    `function shouldUseTeamSearchIndex() {\n  // Staff search stays inside the actor's already-authorized listener scope.\n  return false;\n}`,
  ...
  source = replaceRequired(
    ...
    `      const escalationTarget = runtimeEscalationTarget(currentUser, Object.values(STAFF));\n      if (!escalationTarget) {\n        showTicketAlert(tt("لا يوجد مشرف نشط لاستلام التصعيد.", "No active escalation target is available."), true);\n        return;\n      }\n      const update = {\n        status: "escalated",\n        priority: "emergency",\n        risk: "chargeback",\n        updatedAt: serverTimestamp(),\n        escalatedAt: serverTimestamp(),\n        escalatedBy: currentUser?.id || "",\n        escalatedTo: escalationTarget.ccmsId,\n        escalatedToUid: escalationTarget.employeeUid,\n        escalatedToName: escalationTarget.fullName || escalationTarget.ccmsId,\n        history: addHistory(\n          allTickets.find((x) => x.id === selectedTicketId)?.history,\n          "Escalated to " + (escalationTarget.fullName || escalationTarget.ccmsId) + " (" + escalationTarget.ccmsId + ")",\n          currentUser?.id || ""\n        ),\n      };`,
    'route escalation to active supervisor'
  );
  ...
  if (!source.includes('runtimeEscalationTarget(currentUser, Object.values(STAFF))')) throw new Error('Ticket escalation validation failed.');
