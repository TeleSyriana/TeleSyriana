...
//
// Keeps legacy authentication untouched while enforcing disabled identities at the
// UI/session boundary. Effective identity consumers resolve Reema's historical 9003
// CCMS to permanent employeeUid emp_legacy_9003 with operational CCMS 2002.
...
function blockDisabledLogin(event) {
...
function scheduleEffectiveBoundary() {
  ...
  if (delay <= 0) return;
  // setTimeout has a platform max; this boundary is less than a day away, but keep
  // the clamp explicit so the module stays safe if reused later.
...
function boot() {
  document.addEventListener('submit', blockDisabledLogin, true);
