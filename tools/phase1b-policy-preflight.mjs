...
const itSupport = { employeeUid: 'emp_it', ccmsId: '4001', roleKey: 'it', projectId: '', projectIds: [] };
...
const otherIt = { employeeUid: 'emp_target_it', ccmsId: '4002', roleKey: 'it', projectId: '', projectIds: [], directorySource: 'firestore' };
...
assert.equal(policy.canOpenEmployeesAccounts(itSupport), false);
...
assert.deepEqual(policy.allowedRolesForCreation(ceo), ['acm', 'supervisor', 'hr', 'it', 'agent']);
...
assert.deepEqual(policy.allowedRolesForCreation(itSupport), []);
...
assert.equal(policy.canManageEmployeeTarget(ceo, otherIt), true);
...
assert.equal(policy.canManageEmployeeTarget(acmIPro, otherIt), false);
...
assert.equal(policy.canManageEmployeeTarget(hrMulti, otherIt), false);
assert.equal(policy.canManageEmployeeTarget(hrMulti, otherCeo), false);
assert.equal(policy.canManageEmployeeTarget(itSupport, iProAgent), false);
...
assert.equal(policy.canAssignRoleToTarget(ceo, iProAgent, 'it'), true);
...
assert.doesNotThrow(() => policy.assertProjectAssignmentAllowed(ceo, 'it', []));
assert.throws(() => policy.assertProjectAssignmentAllowed(ceo, 'it', ['ipro']), /must not be assigned operational projects/i);
assert.throws(() => policy.assertProjectAssignmentAllowed(hrMulti, 'it', []), /Only CEO/i);
...
assert.equal(policy.canModifyDirectoryRow(itSupport, iProAgent), false);
