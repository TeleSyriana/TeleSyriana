...
function patchCoreApp(coreSource) {
  ...
  source = replaceBetweenRequired(source, '// Demo users\n', 'function hasRoleAtLeast(user, role) {', '', 'legacy USERS and role helpers');
  ...
  source = replaceBetweenRequired(source, savedSessionStart, savedSessionEnd, savedSessionReplacement, 'saved-session directory refresh');
  ...
  source = replaceRequired(source, '/* --------------------------- Widgets (Clock/Date) ------------------------ */', employeeWatcher + '/* --------------------------- Widgets (Clock/Date) ------------------------ */', 'current employee watcher function');

  source = replaceRequired(source, 'function finishInit(now) {\n  if (canViewTeamDashboard(currentUser)) subscribeSupervisorDashboard();', 'function finishInit(now) {\n  subscribeCurrentEmployeeAccount();\n  if (canViewTeamDashboard(currentUser)) subscribeSupervisorDashboard();', 'start current employee watcher');
  ...
  source = replaceRequired(source, '  if (role !== "agent") return [{ key: "team", source: base }];', '  if (role !== "agent") return [{ key: "team", source: query(base, where("status", "in", ["open", "waiting_customer", "waiting_courier", "waiting_supplier", "escalated", "urgent"])) }];', 'active-only home ticket calendar');
  ...
  source = replaceRequired(source, '  if (cached.name) {\n    currentUser = { ...currentUser, name: cached.name, profilePhoto: cached.profilePhoto || currentUser.profilePhoto || "" };\n    localStorage.setItem(USER_KEY, JSON.stringify(currentUser));\n    if (nameEl) nameEl.value = cached.name;\n  }', '  if (cached.profilePhoto) {\n    currentUser = { ...currentUser, profilePhoto: cached.profilePhoto || currentUser.profilePhoto || "" };\n    localStorage.setItem(USER_KEY, JSON.stringify(currentUser));\n  }', 'ignore cached profile name');

  source = replaceRequired(source, '  renderSettingsProfilePhoto(cached.profilePhoto || "", cached.name || currentUser.name);', '  renderSettingsProfilePhoto(cached.profilePhoto || "", currentUser.name);', 'central name in cached profile render');

  source = replaceRequired(source, '      const savedName = d.name || currentUser.name || currentUser.id;\n      const savedProfilePhoto = d.profilePhoto || cached.profilePhoto || "";\n      currentUser = { ...currentUser, name: savedName, profilePhoto: savedProfilePhoto };\n      localStorage.setItem(USER_KEY, JSON.stringify(currentUser));\n      if (nameEl) nameEl.value = savedName;', '      const savedName = currentUser.name || currentUser.id;\n      const savedProfilePhoto = d.profilePhoto || cached.profilePhoto || "";\n      currentUser = { ...currentUser, profilePhoto: savedProfilePhoto };\n      localStorage.setItem(USER_KEY, JSON.stringify(currentUser));\n      if (nameEl) nameEl.value = savedName;', 'ignore Firestore profile name');

  source = replaceRequired(source, '        name: d.name || currentUser.name || "",', '        name: currentUser.name || "",', 'central name in profile cache');

  source = replaceRequired(source, '  const name = document.getElementById("set-name")?.value?.trim() || currentUser.name || currentUser.id;', '  const name = currentUser.name || currentUser.id;\n  if (document.getElementById("set-name")) document.getElementById("set-name").value = name;', 'prevent self profile rename');

  // Blob-loaded modules can finish after the browser's real DOMContentLoaded
  // event. Convert the preserved listener into an idempotent ready-state boot.
  source = replaceRequired(
    source,
    'document.addEventListener("DOMContentLoaded", async () => {',
    'async function bootTeleSyrianaPhase1() {\n  if (window.__TS_APP_PHASE1_BOOTED__) return;\n  window.__TS_APP_PHASE1_BOOTED__ = true;',
    'app DOM boot start'
  );
  source = replaceRequired(
    source,
    '  showLogin();\n});\n\nfunction closeMobileMenu() {',
    '  showLogin();\n}\nif (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bootTeleSyrianaPhase1);\nelse bootTeleSyrianaPhase1();\n\nfunction closeMobileMenu() {',
    'app DOM boot lifecycle'
  );

  if (source.includes('const USERS = {') || source.includes('safeUserPayload(')) throw new Error('Phase 1 loader validation failed: legacy auth code remains.');
  if (!source.includes('authenticateEmployee(id, pw)') || !source.includes('getEmployee(u?.id') || !source.includes('subscribeCurrentEmployeeAccount()')) throw new Error('Phase 1 loader validation failed: directory auth was not injected.');
  if (!source.includes('nameEl.readOnly = true;') || source.includes('name: cached.name, profilePhoto')) throw new Error('Phase 1 loader validation failed: profile name can still override directory identity.');
  if (!source.includes('setInterval(() => updatePresence(false), 60_000)') || !source.includes('pageId === "home" && canViewOnlineNow(currentUser)')) throw new Error('Phase 1 loader validation failed: quota-safe presence lifecycle missing.');
  if (!source.includes('where("status", "in", ["open"')) throw new Error('Phase 1 loader validation failed: Home issue calendar still reads full ticket history.');
  if (!source.includes('window.__TS_APP_PHASE1_BOOTED__')) throw new Error('Phase 1 loader validation failed: ready-state boot missing.');
