...
function subscribeMainToRoom(roomId) {
  ...
  unsubscribeMain = onSnapshot(
    ...
    (snapshot) => {
      ...
      // ✅ beep for new incoming while open (based on docChanges)
      ...
      changes.forEach((ch) => {
        ...
        const me = String(currentUser?.id || "");
        if (from && me && from !== me) {
          playBeepOnce(String(roomId), ms || Date.now());
        }
      });
...
function filterList(listId, selector) {
  ...
  // keep empty placeholders as-is (ms-empty)
...
// user change (login/logout)  ✅ (fixed: ONLY ONE listener)
window.addEventListener("telesyriana:user-changed", () => {
  setCurrentUser();
