...
function patchMessages(coreSource) {
  ...
  source = replaceRequired(
    source,
    '  setCurrentUser();\n  subscribePresenceSidebar();\n  subscribeProfilesSidebar();',
    '  setCurrentUser();\n  bindMessagePageLifecycle();\n  if (currentUser?.id && messagesPageIsActive()) await activateMessagesDirectoryAndRealtime();',
  ...
  source = replaceRequired(
    ...
    'window.addEventListener("telesyriana:user-changed", () => {\n  setCurrentUser();\n  if (messagesPageIsActive()) activateMessagesDirectoryAndRealtime().catch((err) => console.warn("Messages user-change refresh failed", err));\n  else stopMessagePageRealtime();',
